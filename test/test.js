...
tape( 'the function has an arity of 1', function test( t ) {
	t.strictEqual( dnansumkbn.length, 1, 'has expected arity' );
	t.end();
});

tape( 'the function computes the sum of a one-dimensional ndarray, ignoring `NaN` values and using improved Kahan–Babuška algorithm', function test( t ) {
	var x;
	var v;

	x = new Float64Array( [ 1.0, -2.0, -4.0, 5.0, 0.0, NaN, 3.0, 0.0, -3.0, 3.0, NaN ] ); // eslint-disable-line max-len
	v = dnansumkbn( [ vector( x, 11, 1, 0 ) ] );
	t.strictEqual( v, 3.0, 'returns expected value' );

	x = new Float64Array( [ 1.0, -2.0, -4.0, NaN, 5.0, 0.0, 3.0, NaN ] );
	v = dnansumkbn( [ vector( x, 8, 1, 0 ) ] );
	t.strictEqual( v, 3.0, 'returns expected value' );

	x = new Float64Array( [ -4.0, NaN, -4.0 ] );
	v = dnansumkbn( [ vector( x, 3, 1, 0 ) ] );
	t.strictEqual( v, -8.0, 'returns expected value' );

	x = new Float64Array( [ NaN, 4.0 ] );
	v = dnansumkbn( [ vector( x, 2, 1, 0 ) ] );
